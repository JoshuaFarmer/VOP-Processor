        ...
        for i in range(len(op1)):
                ...
                for x in range(8):
                        if str(op[i]) == f"W{x}":
                                ...
                                idx.append(res[1][0])
                                break
                        elif str(op[i]).split("[")[0] == f"NEAR":
                                res = decode("TEMP:\\" + str(op[i])[4:], labels, worry, lineNum)
                                retstr += "NEAR" + res[0].split(":")[1]
                                idx.append(res[1][0])
                                break
                        elif str(op[i])[0] == f"[":
                                res = decode("TEMP:\\" + str(op[i])[1:-1], labels, worry, lineNum)
                                retstr += "[" + res[0].split(":")[1] + "]"
                                if len(res[1]) == 0:
                                        break
                                else:
