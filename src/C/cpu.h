...
void executeNext() {
	...
	switch (instruction) {
		...
		case POKE_Rn: {
			...
			poke(Rn[idx0], Sn[idx1], P1);
			} break;
		case PEEK_Rn: {
			int idx0 = (fetch(PC++, P0)) & 0xF;
			int idx1 = (fetch(PC++, P0)) & 0xF;
			Rn[idx0] = peek(Sn[idx1], P1);
			} break;
		case PUSH_i8: {
			int val = fetch(PC++, P0);
			push(val);
		...
		case POP_F:
			...
			carry          = value & 2;
			parity         = value & 4;
			interrupt_mask = value & 8;
			data_format    = value & 16;
			} break;
...
U_RET:
		...
		case INC_Rn_AT: {
			int idx = fetch(PC++, P0) & 0xF;
			int old = peek(Rn[idx], P1);
			poke(old+1, Rn[idx], P1);
		} break;
		...
		case DEC_Sn_AT: {
			int idx = fetch(PC++, P0) & 0xF;
			int old = peek(Rn[idx], P1);
			poke(old-1, Sn[idx], P1);
		} break;

		case INC_Sn_AT: {
			int idx = fetch(PC++, P0) & 0xF;
			int old = peek(Sn[idx], P1);
			poke(old+1, Sn[idx], P1);
		} break;
